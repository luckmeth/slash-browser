...
async function assertDropdownRendered(window: BrowserWindowController): Promise<void> {
  ...
  // Polled rather than slept on. A fixed wait races the overlay's first paint —
  // this probe reported "expected 2 rows, found 0" once under load and passed
  // on the very next run, which is the worst thing a probe can do: a check that
  // cries wolf teaches everyone to ignore the whole family of them.
  let rows = 0
  for (let attempt = 0; attempt < 40; attempt += 1) {
    rows = (await overlay.executeJavaScript(
      `document.querySelectorAll('[role="option"]').length`
    )) as number
    if (rows === suggestions.length) break
    await delay(100)
  }
  const text = (await overlay.executeJavaScript(
    `(document.body.innerText || '').replace(/\\n/g, ' | ').slice(0, 120)`
...
export async function runStreamCapture(
  ...
  const run = (args: string[]): void => {
    // `cwd: work` is load-bearing for the DASH step. ffmpeg's dash muxer derives
    // the segment directory by splitting the manifest path on `/` — a Windows
    // path with backslashes leaves it empty, so every chunk lands in the
    // *process's* working directory instead. The manifest was written correctly
    // and the segments were 404, fifteen files deep in the repository root.
  ...
  run([
    ...
    // A keyframe every two seconds, or `-c copy` cannot cut the video into
    // segments at all: ffmpeg wrote *one* chunk for twelve seconds of picture
    // while declaring six, and the DASH probe then 404'd on segment two.
  ...
  // A real MPEG-DASH presentation: separate video and audio representations,
  // a SegmentTemplate, and initialisation segments — which is exactly the shape
  // the parser claims to handle, generated by something that is not us.
  run([
    '-i', source,
    '-map', '0:v', '-map', '0:a',
    '-c', 'copy',
    '-f', 'dash', '-seg_duration', '2', '-use_template', '1', '-use_timeline', '0',
    // Relative, and resolved against `cwd` above — see the note on `run`.
    'manifest.mpd'
  ])
  ...
  // --- 3. MPEG-DASH, assembled and joined ------------------------------------
  //
  // The parser has unit tests against manifest text. This is the other half: a
  // manifest ffmpeg wrote, served over HTTP, planned by the real dispatcher,
  // fetched by the real queue, and read back with a duration and a stream count
  // — because a DASH download that quietly drops the audio representation still
  // produces a file that opens and plays.
  const dashId = hooks.enqueue(`${origin}/manifest.mpd`)
  const dash = await settle(dashId)
  log.info(`stream probe [dash]: state=${dash.state} error=${dash.error ?? 'none'}`)
  log.info(`stream probe [dash]: file → ${inspect(dash.savePath)}`)
  log.info(
    `stream probe [dash]: parts left behind? ` +
      `${existsSync(`${dash.savePath}.video.part`) || existsSync(`${dash.savePath}.audio.part`)}`
  )
  log.info('stream probe [dash]: source was 12s, video h264, audio aac. Anything else is a fail.')
...
/**
 * A picture of the settings screen, because a redesign cannot be reviewed by
 * reading its diff. Opens the panel, waits for it to settle, and captures.
 */
/** A picture of the advertise page, for reviewing the rate card and mocks. */
export async function runAdvertiseShot(
  window: BrowserWindowController,
  outputPath: string
): Promise<void> {
  const activeId = await waitForActiveTab(window)
  ...
  if (chrome) {
    await chrome
      .executeJavaScript(`window.browser.invoke('settings:update', { onboardingCompleted: true })`)
      .catch(() => null)
    await delay(700)
  }
  const overlay = window.overlay.webContents
  ...
  if (activeId) window.tabs.navigate(activeId, 'slash://advertise')
  await delay(2000)

  // Tall, so the whole rate card is in one frame rather than the first two rows.
  const bounds = window.browserWindow.getBounds()
  window.browserWindow.setBounds({ ...bounds, height: 1600 })
  await delay(1400)
  await captureWindowTo(window, outputPath)
  window.browserWindow.setBounds(bounds)
  log.info(`advertise shot: wrote ${outputPath}`)
  if (process.env['SLASH_PROBE_EXIT']) app.quit()
}

/**
 * The rewards page, as somebody who has never heard of Slash Coin sees it.
 *
 * The default state is the one that matters here - opted out, signed out - so
 * this deliberately changes no setting before shooting. `scrollTo` captures the
 * lower sections too, because a marketing page that is only ever checked at the
 * top is one whose second half nobody has looked at.
 */
export async function runRewardsShot(
  window: BrowserWindowController,
  outputPath: string
): Promise<void> {
  const activeId = await waitForActiveTab(window)
  ...
  if (activeId) window.tabs.navigate(activeId, 'slash://rewards')
  await delay(2200)
  ...
  const scroll = Number(process.env['SLASH_SHOT_SCROLL'] ?? '0')
  if (scroll > 0 && chrome) {
    await chrome
      .executeJavaScript(
        `(() => { const el = document.querySelector('.glass-page'); if (el) el.scrollTop = ${scroll}; return true })()`
      )
      .catch(() => false)
    // Long enough for the reveal transitions to finish, or the shot catches
    // half the page mid-fade and looks like a rendering fault.
    await delay(1600)
  }

  await captureWindowTo(window, outputPath)
  window.browserWindow.setBounds(bounds)
  log.info(`rewards shot: wrote ${outputPath}`)
  if (process.env['SLASH_PROBE_EXIT']) app.quit()
}

export async function runSettingsShot(
  window: BrowserWindowController,
  outputPath: string
): Promise<void> {
  await waitForActiveTab(window)
  // A fresh probe profile has not seen the walkthrough, and it sits over the
  // whole window — the first capture was a picture of the onboarding modal.
  const chrome = window.privilegedContents()[0]
  if (chrome) {
    await chrome
      .executeJavaScript(`window.browser.invoke('settings:update', { onboardingCompleted: true })`)
  ...
  // The walkthrough is already mounted by the time the setting lands, so it has
  // to be dismissed the way a person would: by pressing its own Skip.
  const overlay = window.overlay.webContents
  if (overlay) {
    await overlay
      .executeJavaScript(
  ...
  ipcBroadcastUiCommand(window, 'open-settings')
  await delay(2200)

  // Optionally jump to a category, so a toggle-heavy screen can be reviewed
  // rather than only the first one.
  const wanted = process.env['SLASH_SETTINGS_CATEGORY']
  if (wanted && chrome) {
    await chrome
      .executeJavaScript(
        `(() => {
  ...
  await captureWindowTo(window, outputPath)
  log.info(`settings shot: wrote ${outputPath}`)
  if (process.env['SLASH_PROBE_EXIT']) app.quit()
}

/**
 * Every advertising placement, rendered and photographed.
 *
 * All four have been built for a long time and none had ever been seen. This
 * stands up a fixture endpoint of the shape `docs/sponsored-tiles.md`
 * documents, serves one creative per placement, and captures each surface —
 * plus the settings switch that belongs to the user and the card that invites
 * somebody to buy the inventory.
 *
 * The fixture obeys the same rules a paying advertiser would: images are
 * `data:` URLs because `acceptCreative` rejects remote ones (a remote image is
 * a per-impression request to the sponsor, which is a tracking pixel wearing a
 * hat), and one creative is deliberately malformed so the screenshots show the
 * rules refusing it rather than only the happy path.
 */
export async function runAdShowcase(
  window: BrowserWindowController,
  outputDir: string,
  probe: {
    setEndpoint: (url: string) => void
  ...
  const check = (name: string, passed: boolean, detail: string): void => {
    ...
    log[passed ? 'info' : 'error'](`ad showcase [${name}]: ${passed ? 'PASS' : 'FAIL'} — ${detail}`)
  }

  // A 1x1 PNG. Stretched by each placement's own CSS, which is what a real
  // creative's artwork would be doing anyway.
  const swatch =
      ...
      res.end(
        JSON.stringify({
          ...
          tiles: [
            {
              id: 'ad-background',
              sponsor: 'Northwind Studio',
              headline: 'Design tools that keep up',
              body: 'A full takeover of the start page.',
              image: swatch,
              clickUrl: 'https://example.com/northwind',
              placement: 'background'
            },
            {
              id: 'ad-banner',
              sponsor: 'Halden Coffee',
              headline: 'Roasted this week, not last season',
              body: 'A wide panel, visible without owning the page.',
              image: swatch,
              clickUrl: 'https://example.com/halden',
              placement: 'banner'
            },
            {
              id: 'ad-tile',
              sponsor: 'Kestrel Books',
              headline: 'Second-hand, first editions',
              body: 'The small labelled card.',
              image: swatch,
              clickUrl: 'https://example.com/kestrel',
              placement: 'tile'
            },
            {
              id: 'ad-notice',
              sponsor: 'Meridian Rail',
              headline: 'Off-peak fares, booked in advance',
              body: 'A dismissible strip in the browser\'s own chrome.',
              image: '',
              clickUrl: 'https://example.com/meridian',
              placement: 'notice'
            },
            {
              id: 'ad-rail-left',
              sponsor: 'Vector Type',
              headline: 'A typeface for long documents',
              body: 'Gutter card, left.',
              image: swatch,
              clickUrl: 'https://example.com/vector',
              placement: 'rail'
            },
            {
              id: 'ad-rail-right',
              sponsor: 'Pike & Co',
              headline: 'Notebooks that lie flat',
              body: 'Gutter card, right.',
              image: swatch,
              clickUrl: 'https://example.com/pike',
              placement: 'rail'
            },
            {
              // Refused on purpose: a remote image is a request to the sponsor
              // on every impression, which is the tracking this design exists
              // to make impossible.
              id: 'ad-rejected-remote-image',
              sponsor: 'Tracker Co',
              headline: 'This one should never render',
              image: 'https://tracker.example/pixel.png',
              clickUrl: 'https://example.com/tracker',
              placement: 'tile'
            }
          ]
        })
  ...
  try {
    ...
    probe.setEndpoint(`${origin}/batch`)
    await probe.refresh()
    await delay(800)
    ...
    check(
      'batch-accepted',
      status.cached === 6,
      `${status.cached} creative(s) cached — the one with a remote image was refused`
    )
    check(
      'every-placement-filled',
      !!status.background && !!status.banner && !!status.tile && !!status.notice && status.rails.length === 2,
      `background=${status.background?.id} banner=${status.banner?.id} tile=${status.tile?.id} ` +
        `notice=${status.notice?.id} rails=[${status.rails.map((r) => r.id).join(', ')}]`
    )

    // ---- the start page, with the background takeover ---------------------
    // A *new* tab, not a navigation of the existing one. The start page reads
    // sponsor status once when it mounts, and the tab open at launch mounted
    // before this batch existed — navigating it to the same URL does not
    // remount it, so it kept showing the empty status it read first.
    //
    // Reloading is not the answer either: the start page is an internal page
    // drawn by the chrome document with no view attached, and reloading the tab
    // attaches a real one, so the chrome-drawn page stops showing entirely.
    window.tabs.create({ url: NEW_TAB_URL })
    await delay(2800)

    // `activeTab.contents` is **null** on the start page: an internal page has
    // no `WebContentsView` at all — the chrome document shows through the
    // content hole instead. So there is no page-side document to script here,
    // and the placements below the fold cannot be scrolled to from this side.
    // Captured as-is rather than faked; see the note in the report.
    const page = window.tabs.activeTab?.contents ?? null
    log.info(`ad showcase: start page has ${page ? 'a view' : 'no view (internal page)'}`)
    await captureWindowTo(window, path.join(outputDir, '1-background-takeover.png'))

    // ---- every placement at once, by making the window tall ---------------
    //
    // The start page is an internal page: it has **no `WebContentsView`** — the
    // chrome document paints it through the content hole — so there is no
    // document to scroll from out here. `activeTab.contents` is null and the
    // chrome's own body does not contain it either.
    //
    // The capture is a picture of the *window*, though, so the way to see what
    // is below the fold is simply to make the window taller. No DOM access
    // needed, and it shows the takeover, the banner and the tile together,
    // which is what somebody evaluating the inventory wants to see anyway.
    const originalBounds = window.browserWindow.getBounds()
    window.browserWindow.setBounds({ ...originalBounds, height: 1500 })
    await delay(1600)
    await captureWindowTo(window, path.join(outputDir, '2-all-placements.png'))
    window.browserWindow.setBounds(originalBounds)
    await delay(600)

    // ---- the settings the user controls -----------------------------------
    ipcBroadcastUiCommand(window, 'open-settings')
    await delay(1800)
    if (chrome) {
      await chrome
        .executeJavaScript(
          `(() => {
             const b = [...document.querySelectorAll('nav button')].find((x) => (x.textContent||'').trim() === 'Earning');
    ...
    await captureWindowTo(window, path.join(outputDir, '3-settings-earning.png'))
    ipcBroadcastUiCommand(window, 'close-panel')
    await delay(600)

    // ---- billing: the counts an operator sells against --------------------
    const tileId = status.tile?.id ?? 'ad-tile'
    probe.impression(tileId)
    probe.impression(tileId)
    probe.click(tileId)
    await probe.refresh()
    await delay(800)

    const reported = reports.join(' ')
    check(
      'counts-are-reported',
      reported.includes(tileId),
      reports.length > 0
        ? `endpoint received ${reports.length} report(s) naming ${tileId}`
        : 'nothing was reported back'
    )
  } catch (error) {
    check('ran', false, error instanceof Error ? error.message : String(error))
  } finally {
    // Leave the profile as it was found: no endpoint, nothing cached, switch off.
    probe.clear()
    probe.disable()
    await new Promise<void>((resolve) => {
  ...
  log.info(`ad showcase: ${failures === 0 ? 'PASS' : `FAIL (${failures})`} — images in ${outputDir}`)
  if (process.env['SLASH_PROBE_EXIT']) app.quit()
}
