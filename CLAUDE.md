...
| Block YouTube's video ads by filtering requests | Ad segments stream from the same `googlevideo.com/videoplayback` URL as the video, and the ad breaks arrive as a *field in the watch page's HTML*, not as a request | `YouTubeAdFilter` runs one script in YouTube's own JavaScript context (CDP `addScriptToEvaluateOnNewDocument`) and deletes `adPlacements`/`playerAds`/`adSlots`. **This is the only code in Slash that executes in a page's main world.** Gated on hostname, scoped to three fields, switchable off via `blockYouTubeVideoAds`, and it yields to DevTools. It does not touch creator-read sponsor segments, which are part of the video itself |
